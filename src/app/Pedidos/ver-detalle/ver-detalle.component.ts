...
export class VerDetalleComponent {
  ...
  async generarFactura(): Promise<void> {
    ...
    const agregarEncabezado = async () => {
      try {
        const { base64, width, height } = await convertirImgBase64(
          'assets/ProTech.png'
        );
      ...
      y += 5;
      doc.text('https://pro-tech-mu.vercel.app/', margenX, y);
      y += 5;
      doc.text('protech0778@gmail.com | (+57) 314 331 2325', margenX, y);
      y += 7;
    ...
    const agregarDatosPedido = () => {
      ...
      const valores = [
        ...
        `${this.pedido.direccion?.ciudad || 'No disponible'}${
          this.pedido.direccion?.departamento
            ? ', ' + this.pedido.direccion.departamento
            : ''
        }`,
      ];
      ...
      for (let i = 0; i < etiquetas.length; i++) {
        ...
        const textoValor = doc.splitTextToSize(valor, maxWidthValor);
        const altoValor = textoValor.length * lineHeight;
        ...
        doc.setTextColor(30);
        doc.setFont('helvetica', 'normal');
        doc.text(textoValor, margenX + 50, y);

        // Incrementa `y` según el alto del valor (texto multilínea)
        y += altoValor + 0;
      }

      // Línea divisoria después de los datos
      y += 4;
    ...
    const agregarProductoConImagen = async (item: any, index: number) => {
      ...
      doc.text(formatMoneda(subtotal), colSubtotalX, textPosY, {
        align: 'right',
      });
    ...
    const imprimirPie = () => {
      ...
      doc.text(formatMoneda(this.pedido.total), colSubtotalX, y, {
        align: 'right',
      });
      ...
      for (let i = 1; i <= paginas; i++) {
        ...
        doc.text(
          'Gracias por su compra - https://pro-tech-mu.vercel.app/',
          margenDerecho,
          paginaAlto - 10,
          {
            align: 'right',
    ...
    await this.pedido.productos.reduce(
      async (prev: Promise<void>, item: any, i: number) => {
        await prev;
    ...
    imprimirPie();
    doc.save(
      `Factura-${this.pedido.usuario?.nombre || 'cliente'}-${
        this.pedido._id
      }.pdf`
    );
  }
